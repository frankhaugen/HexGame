...
> **Note:** This is an architecture specification for software design. It is **not** an IETF RFC and has not been reviewed or published by the IETF.

## Abstract

HexGame describes a hexagonal, authoritative-core architecture for games: an application core owns simulation state and rules; inputs arrive as technology-neutral commands; each tick advances state and yields snapshots, domain events, and adapter-executed effects; rendering, transport, persistence, and tooling remain replaceable adapters. The model supports local play, dedicated servers, tests, replays, and bots without duplicating game logic across runtimes. This document defines normative requirements and informative guidance for systems that claim alignment with HexGame.
...
- [5. Required Layers](#5-required-layers)
- [6. Dependency Rule](#6-dependency-rule)
- [7. Application Core Contract](#7-application-core-contract)
- [8. Commands](#8-commands)
- [9. Authoritative State](#9-authoritative-state)
- [10. Tick](#10-tick)
- [11. Snapshot](#11-snapshot)
- [12. Events](#12-events)
- [13. Effects](#13-effects)
- [14. Rendering Adapter](#14-rendering-adapter)
- [15. Input Adapter](#15-input-adapter)
- [16. Networking](#16-networking)
- [17. Local Single-Player](#17-local-single-player)
- [18. Hosting Modes](#18-hosting-modes)
- [19. Persistence](#19-persistence)
- [20. Assets](#20-assets)
- [21. Testing](#21-testing)
- [22. Replay](#22-replay)
- [23. Bot and Simulation Runners](#23-bot-and-simulation-runners)
- [24. Tooling and Editors](#24-tooling-and-editors)
- [25. Stride Lite Adapter Layer](#25-stride-lite-adapter-layer)
- [26. Anti-Patterns](#26-anti-patterns)
- [27. Compliance Levels](#27-compliance-levels)
- [28. Recommended Project Layout](#28-recommended-project-layout)
- [29. Recommended Minimal Contracts](#29-recommended-minimal-contracts)
- [30. Summary](#30-summary)
- [31. Security Considerations](#31-security-considerations)
- [32. IANA Considerations](#32-iana-considerations)
- [33. Informative References](#33-informative-references)
...
**Game application core** (or **core**) means the combined domain and application orchestration that owns authoritative simulation state and exposes the explicit application boundary (for example, start, tick, save, load). **Adapters** connect external technologies to that boundary.
...
HexGame is an architecture pattern, not a wire protocol. Implementations remain responsible for their own threat models. The following considerations apply to typical systems built in this style:

- **Authority and validation:** Multiplayer servers that follow this document MUST NOT treat client-supplied state as authoritative. Command validation, rate limits, and replay protection are implementation responsibilities at the host and transport adapters.
- **Serialization and trust:** Snapshots, events, effects, and save data cross trust boundaries when sent over a network or read from disk. Implementations SHOULD use explicit schema versioning, integrity checks where appropriate, and safe deserialization practices at adapter boundaries.
- **Tooling and editors:** Adapters that expose editing or debug interfaces SHOULD authenticate and authorize access; they MUST NOT bypass core validation rules for convenience.
- **Effects execution:** Effect executors run with the privileges of the host process. Untrusted game content or mod pipelines SHOULD be constrained by the same validation layers as commands and assets.

## 32. IANA Considerations

This document has no IANA actions. It does not define a registered protocol, port, media type, or URI scheme.

## 33. Informative References

The patterns below are cited for reader background only; they are not normative requirements of this specification.

- Alistair Cockburn, *Hexagonal Architecture* (ports and adapters).
- Robert C. Martin, *Clean Architecture* (dependency rule and use-case orchestration).
- Eric Evans, *Domain-Driven Design* (entities, value objects, bounded contexts).
- Gregor Hohpe and Bobby Woolf, *Enterprise Integration Patterns* (messaging and event-style integration at boundaries).
- Common multiplayer practice: server-authoritative simulation with clients sending intent (industry pattern; see also discussion of prediction and confirmation in Section 16 of this document).
